...
const EditProfile = () => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/user/uploadPicture", data);
  ...
  const handleSaveRegistration = async () => {
    ...
    if (check) {
      ...
      try {
        console.log("form:", form);
        const { data } = await axios.post("http://localhost:5000/user/registered/saveRegDetails", form);
  ...
  const handleSaveEducation = async () => {
    ...
    if (check1 && check2) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post("http://localhost:5000/user/registered/saveEduDetail", form);
        toast.success("Details saved successfully");
      } catch (error) {
        console.log(error);
        if (error?.response?.data?.message) {
  ...
  const handleSaveAdditional = async () => {
    ...
    if (check1 && check2 && check3) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post("http://localhost:5000/user/registered/saveAddDetail", form);
        toast.success("Details saved successfully");
      } catch (error) {
        console.log(error);
        if (error?.response?.data?.message) {
