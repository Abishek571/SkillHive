...
const Register = () => {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        const { data } = await axios.get("http://localhost:5000/user/unregistered/getDetails");
  ...
  const handleSaveRegistration = async () => {
    ...
    if (check) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post("http://localhost:5000/user/unregistered/saveRegDetails", form);
  ...
  const handleSaveEducation = async () => {
    ...
    if (check1 && check2) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post("http://localhost:5000/user/unregistered/saveEduDetail", form);
        toast.success("Details saved successfully");
      } catch (error) {
        console.log(error);
        if (error?.response?.data?.message) {
  ...
  const handleSaveAdditional = async () => {
    ...
    if (check1 && check2 && check3) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post("http://localhost:5000/user/unregistered/saveAddDetail", form);
        toast.success("Details saved successfully");
      } catch (error) {
        console.log(error);
        if (error?.response?.data?.message) {
  ...
  const handleSubmit = async () => {
    ...
    if (check1 && check2 && check3) {
      setSaveLoading(true);
      try {
        const { data } = await axios.post("http://localhost:5000/user/registerUser", form);
